...
export default function treeToList(tree, callback, level = 0, result = [], maxLevel) {
  for (const item of tree) {
    ...
    if (item.children?.length) {treeToList(item.children, callback, maxLevel && level>=maxLevel ? level : level + 1 , result); result.push(callback({status: 'empty', _id:item._id, text: "", dateCreate:"0000-01-01T00:00:00.000Z"},level+1))};
    if (!item.children?.length) result.push(callback({status: 'empty', _id:item._id, text: "", dateCreate:"0000-01-01T00:00:00.000Z"},level+1));
  }
  return result;
}
