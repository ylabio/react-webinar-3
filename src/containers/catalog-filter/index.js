...
function CatalogFilter() {
  ...
  const select = useSelector(state => ({
    ...
    categoryValue: state.catalog.params.category,
  }));
  ...
  const options = {
    category: useMemo(() =>
      ([      
         ...updateCategoryList(select.category)
    ]), [select.category]),
    sort: useMemo(() => ([
  ...
  return (
    <SideLayout padding='medium'>
      <Select options={options.category} value={select.categoryValue} onChange={callbacks.onCategory}/>
