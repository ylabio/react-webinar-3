  ...
  return (
    <div className={cn()}>
      ...
      <div className={cn('right')}>
        <div className={cn('cell')}>{numberFormat(props.item.price)} {languages[props.lang].price}</div>
        <div className={cn('cell')}>{numberFormat(props.item.amount || 0)} {languages[props.lang].count}</div>
        <div className={cn('cell')}>
          <button onClick={callbacks.onRemove}>{languages[props.lang].delete}</button>
        </div>
      </div>
    </div>
  )
